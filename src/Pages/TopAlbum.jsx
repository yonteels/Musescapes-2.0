...
function TopAlbum() {
    const { topAlbum } = useOutletContext()
    ...
    return (
        <div>
            {/* Banner */}
            <div className='relative w-full h-48 bg-gradient-to-r from-blue-900 to-stone-700 rounded-2xl mb-6 flex items-end p-6 overflow-hidden'>
                ...
                <div>
                    ...
                    <h1 className='text-4xl font-black text-amber-50'>Top Album</h1>
            ...
            {/* Track count */}
            <p className='text-stone-500 text-sm mb-4'>{topAlbum?.length || 0} songs</p>
            ...
            {!topAlbum || topAlbum.length === 0 ? (
                ...
                <>
                    {topAlbum.slice(0, visibleCount).map((track, index) => (
                        <div
                            key={index}
                    ...
                    {visibleCount < topAlbum.length && (
                        ...
                        >
                            Show more ({topAlbum.length - visibleCount} remaining)
                        </button>
                    )}

                    {visibleCount >= topAlbum.length && topAlbum.length > 10 && (
                        <button
