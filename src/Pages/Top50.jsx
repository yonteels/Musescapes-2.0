        ...
        <div>
            ...
            <div className='relative w-full h-48 bg-gradient-to-r from-stone-900 to-stone-700 rounded-2xl mb-6 flex items-end p-6 overflow-hidden'>
                <div className='absolute inset-0 opacity-10 flex items-center justify-center'>
                    <span className='text-[200px] font-black text-white select-none'>50</span>
                </div>
                <div>
                    <p className='text-stone-400 text-sm uppercase tracking-widest mb-1'>Playlist</p>
                    <h1 className='text-4xl font-black text-amber-50'>Global Top 50</h1>
                    <p className='text-stone-400 text-sm mt-1'>The most played songs in the world right now</p>
                </div>
                <button
            ...
            {/* Track count */}
            <p className='text-stone-500 text-sm mb-4'>{topTracks?.length || 0} songs</p>
            ...
            {!topTracks || topTracks.length === 0 ? (
                ...
                <>
                    {topTracks.slice(0, visibleCount).map((track, index) => (
                        <div
                            key={index}
                    ...
                    {visibleCount < topTracks.length && (
                        ...
                        >
                            Show more ({topTracks.length - visibleCount} remaining)
                        </button>
                    )}

                    {visibleCount >= topTracks.length && topTracks.length > 10 && (
                        <button
